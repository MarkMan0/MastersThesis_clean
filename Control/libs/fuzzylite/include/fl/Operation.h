...
namespace fl {
  ...
  public:
     ...
     */
    static std::string findReplace(const std::string& str, const std::string& find, const std::string& replace,
...
namespace fl {
  ...
  inline std::string Operation::findReplace(const std::string& str, const std::string& find, const std::string& replace,
                                            bool replaceAll) {
    ...
    do {
      ...
      if (nextIndex != std::string::npos) result << replace;
      fromIndex = nextIndex + find.size();
    } while (replaceAll and nextIndex != std::string::npos);
    return result.str();
  }

  inline std::string Operation::replaceFirst(const std::string& str, const std::string& find,
                                             const std::string& replace) {
    return findReplace(str, find, replace, false);
  }

  inline std::string Operation::replaceAll(const std::string& str, const std::string& find,
                                           const std::string& replace) {
    return findReplace(str, find, replace, true);
  }

  inline std::vector<std::string> Operation::split(const std::string& str, const std::string& delimiter,
                                                   bool ignoreEmpty) {
  ...
  inline std::string Operation::trim(const std::string& text) {
    ...
    if (not(std::isspace(text.at(0)) or std::isspace(text.at(text.size() - 1)))) return text;
  ...
  inline std::string Operation::format(const std::string& text, int matchesChar(int), const std::string& replacement) {
    std::ostringstream ss;
  ...
  inline scalar Operation::toScalar(const std::string& x) {
    ...
    _nan << fl::nan;
    if (x == _nan.str() or x == "nan") return fl::nan;

    std::ostringstream pInf;
    pInf << fl::inf;
    if (x == pInf.str() or x == "inf") return fl::inf;

    std::ostringstream nInf;
    nInf << (-fl::inf);
    if (x == nInf.str() or x == "-inf") return -fl::inf;
